...
def GetDataList_AVHRR_AMI(file_list_all):
    ...
    for file in file_list_all:
        ...
        elif file.find('GPP') != -1:
            file_list_GPP.append(file)
        elif file.find('NPP') != -1:
            file_list_NPP.append(file)

    return file_list_RVI, file_list_NDVI, file_list_SAVI, file_list_EVI, file_list_LAI, file_list_GPP, file_list_NPP
...
def GetDataList(file_list_all):
    ...
    return file_list_RVI, file_list_NDVI, file_list_SAVI, file_list_GVI, file_list_EVI, file_list_LAI, file_list_GPP, file_list_NPP
...
def Write_nc(data, lat, lon, Mon_Dir, Type, name, date):
    Output_path = Mon_Dir + '/' + 'Year-' + Type + name + date + '.nc'
