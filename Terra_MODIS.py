...
def Create_VRT(dataset, VRT_path):
    ...
    vrtFile = gdal.Translate(VRT_path + 'TERRA.vrt', data, format='vrt')


# 修改VRT文件
def modify_VRT(VRT_path, mark):
    tree = ET.parse(VRT_path + 'TERRA.vrt')
    root = tree.getroot()
...
def GetData(fileName, flag, filePath_TIF, hour):
    ...
    GeoCorrection(output_file_path + 'Terra_Data1_' + hour + '.tif', filePath_TIF + 'Terra.vrt', 0.005, -999)
    # data2(TOA_1000(26),BT_1000(31,32)
    GetDataTiff(data2, filePath_TIF)
    Create_VRT(filePath_TIF + 'data.tif', filePath_TIF)
    modify_VRT(filePath_TIF, True)
    GeoCorrection(output_file_path + 'Terra_Data2_' + hour + '.tif', filePath_TIF + 'Terra.vrt', 0.01, -999)
    # data3((TOA_500(26),BT_500(31,32))
    GeoCorrection(output_file_path + 'Terra_Data3_' + hour + '.tif', filePath_TIF + 'Terra.vrt', 0.005, -999)
    # angle
    GetDataTiff(angle, filePath_TIF)
    Create_VRT(filePath_TIF + 'data.tif', filePath_TIF)
    modify_VRT(filePath_TIF, False)
    GeoCorrection(output_file_path + 'Terra_angle_' + hour + '.tif', filePath_TIF + 'Terra.vrt', 0.005, 25500)

    # 获取定标系数
    calibration = GetCalibrationCoefficient(hdf)
    # 辐射校正
    band_500, band_1000, band_1000_500 = RadioCorrection(output_file_path + 'Terra_Data1_' + hour + '.tif',
                                                         output_file_path + 'Terra_Data2_' + hour + '.tif',
                                                         output_file_path + 'Terra_Data3_' + hour + '.tif',
                                                         calibration)
    # 获取经纬度
    lat_500, lon_500 = GetGeolocation(output_file_path + 'Terra_Data1_' + hour + '.tif')
    lat_1000, lon_1000 = GetGeolocation(output_file_path + 'Terra_Data2_' + hour + '.tif')
    angle_data = gdal.Open(output_file_path + 'Terra_angle_' + hour + '.tif').ReadAsArray()
    sa_z = angle_data[0]
    sa_a = angle_data[1]
    so_z = angle_data[2]
    so_a = angle_data[3]
...
def TOA_nc(band_500, band_1000, sa_a, sa_z, so_a, so_z, A, lat_500, lon_500, lat_1000, lon_1000, isCloud, Ins_Dir,
           date):
    Output_path = Ins_Dir + '/' + 'TOA(Terra-MODIS)_' + date + '.nc'
